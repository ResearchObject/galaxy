...
class WorkflowRunCrateProfileBuilder:
    ...
    def build_crate(self):
        crate = ROCrate()
        crate.name = f'Invocation of workflow "{self.workflow.name}" on {self.invocation.create_time.isoformat()}'
        crate.description = f'Invocation of Galaxy workflow "{self.workflow.name}" executed at {self.invocation.create_time.isoformat()}. This RO-Crate was automatically generated by Galaxy.'
        self._add_workflows(crate)
        self._add_engine_run(crate)
        self._add_create_action_workflow(crate)
        self._add_create_action_steps(crate)
    ...
    def _add_workflows(self, crate: ROCrate):
        ...
        crate.mainEntity.type.append("HowTo")
    ...
    def _add_create_action_workflow(self, crate: ROCrate):
    ...
    def _add_create_action_steps(self, crate: ROCrate):
        action_entities: List[ContextEntity] = []
        self._add_create_action_steps_recursive(self.invocation.steps, crate, action_entities)

    def _add_create_action_steps_recursive(self, invocation_steps, crate: ROCrate, action_entities: list):
        # actions for each tool execution
        for invocation_step in invocation_steps:
            workflow_step = invocation_step.workflow_step
            if workflow_step.type == "tool":
                # Create a unique CreateAction entity for each step
                create_action = crate.add(
                    ContextEntity(
                        ...
                        properties={
                            "@type": "CreateAction",
                            "name": f"Execute {workflow_step.tool_id}",
                            "startTime": invocation_step.create_time.isoformat(),
                            "endTime": invocation_step.update_time.isoformat(),
                        },
                    )
                )
                # get relevant tool entity
                tool_entity = self.tool_cache[f"{workflow_step.tool_id}:{workflow_step.tool_version}"]
                create_action["instrument"] = tool_entity

            # Handle subworkflows recursively
            elif invocation_step.workflow_step.type == "subworkflow":
                subworkflow_invocation = self.invocation.get_subworkflow_invocation_for_step(
                    invocation_step.workflow_step
                )
                if subworkflow_invocation:
                    self._add_create_action_steps_recursive(subworkflow_invocation.steps, crate, action_entities)
